...
class DefaultLib(FunctionLibraryBase):
    ...
    @staticmethod
    @implementNode(returns=DataTypes.Bool, meta={'Category': 'GenericTypes', 'Keywords': []})
    def makeBool(b=(DataTypes.Bool, False)):
        '''make simple boolean'''
        return b

    @staticmethod
    @implementNode(returns=DataTypes.Int, meta={'Category': 'GenericTypes', 'Keywords': []})
    def makeInt(i=(DataTypes.Int, 0)):
        '''make integer'''
        return i

    @staticmethod
    @implementNode(returns=DataTypes.Float, meta={'Category': 'GenericTypes', 'Keywords': []})
    ...
    # Conversions
    @staticmethod
    @implementNode(returns=DataTypes.Int, meta={'Category': 'Conversion', 'Keywords': []})
    def boolToInt(b=(DataTypes.Bool, False)):
        return int(b)

    @staticmethod
    @implementNode(returns=DataTypes.Bool, meta={'Category': 'Conversion', 'Keywords': []})
