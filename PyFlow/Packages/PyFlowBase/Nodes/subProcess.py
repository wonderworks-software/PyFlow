...
class subProcess(NodeBase):
    ...
    def compute(self, *args, **kwargs):
        ...
        if len(cmd_head) <= 0:
            ...
            opt_except_pin = [self.inExecPin, self.cmd, self.cmd_end, self.cmd_opt]
            cmd_opt_ext = ""
            ...
            self.proc_task = asyncio.get_event_loop().create_task(self._run_cmd(self.proc_task_uuid, cmd))
        
    async def _run_cmd(self, _uuid, cmd):
        if None != self.proc and None == self.proc.returncode:
            self.proc.terminate()
        self.computing.send()
        self.proc = None
        uuid = _uuid
        proc:asyncio.subprocess.Process = await asyncio.create_subprocess_shell(cmd, stdout=asyncio.subprocess.PIPE, stdin=asyncio.subprocess.PIPE)
        self.proc = proc
